...
```{r aula02-exemplo-01, engine='tikz', echo=FALSE, fig.cap="Camisetas X Tamanhos", fig.path='01-aula01/media/latex/' }
...
```{r aula02-exemplo-02, engine='tikz', echo=FALSE, fig.cap="Capital X Países", fig.path='01-aula01/media/latex/'}
\usetikzlibrary{arrows,positioning,shapes,fit,calc}
...
```{r aula02-exemplo-03, engine='tikz', echo=FALSE, fig.cap="camisetas X Tamanhos", fig.path='01-aula01/media/latex/'}
\usetikzlibrary{arrows,positioning,shapes,fit,calc}
...
```{r aula02-exemplo-04, engine='tikz', echo=FALSE, fig.cap="Países X Capitais", fig.path='01-aula01/media/latex/'}
\usetikzlibrary{arrows,positioning,shapes,fit,calc}
...
```{r aula02-exemplo-05, engine='tikz', echo=FALSE, fig.cap="Produtos X Categorias", fig.path='01-aula01/media/latex/'}
\usetikzlibrary{arrows,positioning,shapes,fit,calc}
...
```{r 02-Fundamentos-01-html, eval=FALSE, include=FALSE}
...
```{r 02-Fundamentos-01-doc, eval=FALSE, include=FALSE}
...
```{r 02-Fundamentos-01-exercicios-html, eval=FALSE, include=FALSE}
...
FUNCIONARIOS  -N- PERTENCE
PERTENCE      -1- DEPARTAMENTOS
...
```{r 03-Modelagem-01-aula-html, eval=FALSE, include=FALSE}
...
rmarkdown::render("07-2026-2026-04-07_MySQL-aula.Rmd", output_dir="docs", output_file ="temporario.html" , output_format = "html_document") ; 
...
Para faze-lo devemos criar uma interface entre o Sistema de Gerenciamento de 
Banco de Dados e a página WEB.

### Conceito de CRUD

**CRUD** é um acrônimo para quatro operações fundamentais que um sistema realiza sobre dados em um banco de dados:

![](09-aula09/media/01-CRUD.jpg)
...
.tg .tg-ev4w{background-color:#32cb00;border-color:inherit;font-weight:bold;text-align:left;vertical-align:top}
.tg .tg-0pky{border-color:inherit;text-align:left;vertical-align:top}
.tg .tg-qi4s{background-color:#f8ff00;border-color:inherit;font-weight:bold;text-align:left;vertical-align:top}
</style>
<table class="tg"><thead>
  <tr>
    <th class="tg-6e8n">Operação C.R.U.D.</th>
    <th class="tg-6e8n">Significado em Português</th>
    <th class="tg-6e8n">Operação SQL DML</th>
  </tr></thead>
<tbody>
  <tr>
    <td class="tg-ev4w">CREATE</td>
    <td class="tg-0pky">Criar ⇒</td>
    <td class="tg-qi4s">INSERT</td>
  </tr>
  <tr>
    <td class="tg-ev4w">READ</td>
    <td class="tg-0pky">Ler ⇒</td>
    <td class="tg-qi4s">SELECT</td>
  </tr>
  <tr>
    <td class="tg-ev4w">UPDATE</td>
    <td class="tg-0pky">Atualizar ⇒</td>
    <td class="tg-qi4s">UPDATE</td>
  </tr>
  <tr>
    <td class="tg-ev4w">DELETE</td>
    <td class="tg-0pky">Apagar ⇒</td>
    <td class="tg-qi4s">DELETE</td>
  </tr>
</tbody>
</table>
...
```{plantuml aula-09-diagrama-cliente-servidor-01, plantuml.path="images"}
@startuml
skinparam componentStyle rectangle

package "Denominado Front-end" {
    [Cliente WEB (HTML + JavaScript) ] as Browser
}

package "Denominado Back-end" {
    [Servidor de Aplicação Python] as Server
}

database "Camada de Dados" {
    [Servidor de Banco de Dados - SGBD] as DB
}

Browser <--> Server : ================ Conversa por Protocolo HTTP (transporta Verbos http)
Server  <--> DB     : ================ Conversa por Protocolo TCP  (transporta SQL DML)

note right of Browser : Desenha a interface\ne executa lógica de cliente
note right of Server : Processa a chamada lógica de negócio\ne autenticação
note right of DB : Armazena e gerencia\ndados persistentes
@enduml
```
...
```{plantuml aula-09-diagrama-cliente-servidor-01-mvc, plantuml.path="images"}
@startuml
top to bottom direction
skinparam linetype ortho
skinparam packageStyle frame

title Cliente, Servidor e Banco de Dados para Arquitetura M.V.C.:

package "\n\nNa lógica VIEW\nentra o CLIENTE\n\n" {
    [Cliente HTML + JavaScript] as View
    note bottom of View : Tela de Interfaceamento \n\ncom usuário e\n\nexibição dos dados.
}

package "\n\nNa lógica CONTROLLER\n entra o SERVIDOR\n\n" {
    [Servidor de aplicação Python] as Controller
    note top of Controller : Lógica de controle,\n\nRotas e Intermediação: \n\nExposição da A.P.I.
}

package "\n\nNa lógica MODEL\n entra o Banco de dados\n\n " {
    [O seu Banco de Dados Preferido\n PostgreSQL, MySQL, SQLServer] as Model
    note bottom of Model : Persistência de dados e\n\nregras de negócio da base.
}


View       <----> Controller : => Tela WEB - consultas http
Controller <--> Model : => Consultas SQL (tradicionalmente)
...
## MODELAGEM - Construindo uma aplicação CRUD para interfacear com a tabela "Pessoa"
...
```{plantuml modelo-fisico-continuação, plantuml.path="images"}
@startuml
' Configurações visuais para parecer uma tabela de banco de dados
hide circle
skinparam linetype ortho
skinparam class {
...
title Modelo Físico — Tabela PESSOAS
...
note right of pessoas : O campo 'cpf' é a Chave Primária (PK).\nOs campos com '*' são obrigatórios (NOT NULL).
@enduml
```

## MÃO NA MASSA - Construindo uma aplicação CRUD para interfacear com a tabela "Pessoa"
...
#### USUÁRIO DO BANCO MySQL - CRIAÇÃO
...
#### ESQUEMA DO BANCO MySQL - CRIAÇÃO
...
USE pessoasdb;
```
####  Tabela "pessoas": CRIAÇÃO

##### SQL específico ao MySQL 
...
#### Servidor de Aplicação Python <==> MySQL: CRIAÇÃO

##### Baixar os Pacotes de conexão python <==> MySQL
...
```{r 09-CRUD-01-aula-html, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

rmarkdown::render("09-2026-2026-04-21_CRUD-1-aula.Rmd", output_dir="docs", output_file ="temporario.html" , output_format = "html_document") ; 
...
```{r 09-CRUD-01-exercício-html, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r 10-CRUD-02-exercicio-html, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

rmarkdown::render("10-2026-2026-04-28_CRUD-2-exercicio.Rmd", output_dir="docs", output_file ="temporario.html" , output_format = "html_document") ; 
...
```{r 11-Análise-01-aula-html, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r 11-Análise-02-exercicio-html, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

rmarkdown::render("11-2026-2026-05-05_Analise_01-exercicio.Rmd", output_dir="docs", output_file ="temporario.html" , output_format = "html_document") ; 
...
```{r 12-Análise-02-aula-html, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

rmarkdown::render("12-2026-2026-05-12_Analise_02-aula.Rmd", output_dir="docs", output_file ="temporario.html" , output_format = "html_document") ; 
...
```{r 12-Análise-02-exercicio-html, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
