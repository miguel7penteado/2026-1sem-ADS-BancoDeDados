...
|  |  |
|----|----|
| ![Donald Chamberlain - Criador da linguagem SQL nos laboratórios da IBM](media/04-Donald_Chamberlain.jpg) | ![Raymond Boyce - Criador da linguagem SQL nos laboratórios da IBM](media/04-Raymond_Boyce.jpg){width="239"} |
...
```{r tikz-diagrama12, engine='tikz', echo=FALSE, fig.cap="Diagrama de Montadoras, Veículos e Proprietários"}
\usetikzlibrary{arrows,positioning,shapes,fit,calc}
...
```{r tikz-diagrama12-02, engine='tikz', echo=FALSE, fig.cap="Diagrama de Montadoras, Veículos e Proprietários"}
\usetikzlibrary{arrows,positioning,shapes,fit,calc}
...
[
  ...
  % Estilo das elipses de fundo
  every fit/.style={draw, fill=gray!10, ellipse, inner sep=10pt},
  >=latex,
  node distance = 1.5cm and 4cm
]

% --- Conjunto A (Países) ---
\node [setA, label=left:Brasil]    (a) {};
\node [setA, below=of a, label=left:Argentina] (b) {};
\node [above=1cm of a, font=\bfseries] {Países};

% --- Conjunto B (Capitais) ---
\node [setB, right=of a, label=right:Brasília]    (x) {};
\node [setB, below=of x, label=right:{Buenos Aires}] (y) {};
\node [above=1cm of x, font=\bfseries] {Capitais};

% --- As Setas (Mapeamento) ---
\draw[->, shorten >= 3pt, shorten <= 3pt, thick, gray!80] (a) -- (x);
\draw[->, shorten >= 3pt, shorten <= 3pt, thick, gray!80] (b) -- (y);
